...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Modal for Benchmark comparison */
    .modal-overlay {
      position: fixed;
    ...
    .table-benchmark tr:nth-child(even) { background: #162032; }
    .badge-pass {
...
<body>
  ...
  <aside class="sidebar">
    <div class="brand">
      ...
      <div class="brand-text">
        ...
        <p>K4-Day04 • Team NhomGG (Port 8501)</p>
      </div>
    </div>

    <div class="control-group">
      <label>Phiên bản Agent</label>
      <select id="agentVersion" onchange="onVersionChange()">
    ...
    <!-- Live Version Stats Card -->
    <div class="version-card" id="versionCard">
      <div class="version-card-title">
        <span id="cardVerName">Phiên bản v3 (Tối ưu nhất)</span>
        <span class="badge-pass">Active</span>
      </div>
      <div style="font-size: 11px; color: #94A3B8;" id="cardVerDesc">
        Triage đa nguồn song song, ưu tiên hội thoại đa lượt, tích hợp tool diagnose_network.
      </div>
      <div class="version-stats">
        <div class="stat-item"><span>Case Acc:</span> <b id="statCaseAcc">83.33% (25/30)</b></div>
        <div class="stat-item"><span>Routing:</span> <b id="statRouting">90.00%</b></div>
        <div class="stat-item"><span>Multi-turn:</span> <b id="statMulti">90.00% (9/10)</b></div>
        <div class="stat-item"><span>Group Test:</span> <b id="statGroup">90.00% (9/10)</b></div>
      </div>
    </div>
    ...
    <div class="sidebar-actions">
      <button class="btn-benchmark" onclick="openBenchmarkModal()">
        📊 Xem Kết Quả Benchmark v0–v3
      </button>
    ...
    <header class="chat-header">
      <div class="chat-header-title">
        ...
        <div>
          ...
          <div style="font-size: 11px; color: var(--text-muted);" id="headerSubtitle">Artifact: v3 • Provider: OpenRouter • Port 8501</div>
        </div>
      </div>
  ...
  <!-- Benchmark Modal -->
  <div class="modal-overlay" id="benchmarkModal" onclick="closeBenchmarkModal(event)">
    <div class="modal-content" onclick="event.stopPropagation()">
      <div class="modal-header">
        <h2 style="font-size: 18px; color: #fff;">📊 Bảng Đo Lường & So Sánh Benchmark Thực Tế (v0 ➔ v3)</h2>
        <button class="modal-close" onclick="closeBenchmarkModal()">&times;</button>
      </div>
      <p style="font-size: 12px; color: var(--text-muted); margin-bottom: 12px;">
        Dữ liệu đối chiếu từ các file run JSON đo lường thực tế trên bộ 30 test case chuẩn (Base), 10 case Nhóm (Group), 10 case Mở rộng (Extension) và 12 case An toàn (Adversarial):
      </p>
      ...
      <table class="table-benchmark">
        ...
        <tbody>
          <tr>
            <td><b>v0</b></td>
            <td>Baseline ban đầu (chưa sửa prompt/tool)</td>
            <td>70.0% (21/30)</td>
            <td>76.67%</td>
            <td>80.0%</td>
            <td><span class="badge-pass" style="background:rgba(239,68,68,0.2); color:#F87171;">Baseline</span></td>
          </tr>
          <tr>
            <td><b>v1</b></td>
            <td>Thêm quy tắc thiếu thông tin (missing_info ➔ clarify)</td>
            <td>70.0% (21/30)</td>
            <td><b>83.33%</b> ↗</td>
            <td>80.0%</td>
            <td><span class="badge-pass" style="background:rgba(59,130,246,0.2); color:#60A5FA;">Tăng Routing</span></td>
          </tr>
          <tr>
            <td><b>v2</b></td>
            <td>Ranh giới an toàn tạo vé (wrong_boundary) & required clarify</td>
            <td><b>80.0% (24/30)</b> ↗</td>
            <td><b>90.00%</b> ↗</td>
            <td>80.0%</td>
            <td><span class="badge-pass" style="background:rgba(16,185,129,0.2); color:#34D399;">Khắc phục vé</span></td>
          </tr>
          <tr>
            <td><b>v3</b></td>
            <td>Triage đa nguồn song song & ngữ cảnh đa lượt</td>
            <td><b>83.33% (25/30)</b> 🏆</td>
            <td><b>90.00%</b></td>
            <td><b>90.00% (9/10)</b> ↗</td>
            <td><span class="badge-pass">Tối ưu nhất</span></td>
          </tr>
          <tr>
            <td><b>Group (10c)</b></td>
            <td>10 Test case nhóm (Có Bonus diagnose_network)</td>
            <td><b>90.00% (9/10)</b></td>
            <td><b>90.00%</b></td>
            <td><b>80.00%</b></td>
            <td><span class="badge-pass">Bonus Tool OK</span></td>
          </tr>
          <tr>
            <td><b>Extension</b></td>
            <td>10 Test case mở rộng policy/search_device_info/create_ticket</td>
            <td>40.0% (4/10)</td>
            <td><b>100.0% (1.0)</b></td>
            <td>-</td>
            <td><span class="badge-pass">100% Routing</span></td>
          </tr>
        </tbody>
      </table>

      <div style="margin-top: 16px; padding: 12px; background: #1E293B; border-radius: 8px; font-size: 12px; color: #CBD5E1;">
        💡 <b>Kết luận:</b> Qua 3 vòng lặp, <b>Case Accuracy tăng +13.33%</b> (từ 70% lên 83.33%), <b>Tool Routing tăng +13.33%</b> (từ 76.67% lên 90%), và <b>Multi-turn Accuracy tăng +10%</b> (từ 80% lên 90%). Tool mở rộng <code>diagnose_network</code> đạt 100% PASS trong các bài test đơn và song song.
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    const versionData = {
      v3: { name: "Phiên bản v3 (Tối ưu nhất)", desc: "Triage đa nguồn song song, ưu tiên ngữ cảnh đa lượt, tích hợp tool diagnose_network.", acc: "83.33% (25/30)", routing: "90.00%", multi: "90.00% (9/10)", group: "90.00% (9/10)" },
      v2: { name: "Phiên bản v2", desc: "Thêm ranh giới xác nhận vé (confirmed=True) và required response_type cho clarify.", acc: "80.00% (24/30)", routing: "90.00%", multi: "80.00%", group: "80.00%" },
      v1: { name: "Phiên bản v1", desc: "Bổ sung quy tắc xử lý khi thiếu định danh bắt buộc (gọi clarify thay vì đoán mò).", acc: "70.00% (21/30)", routing: "83.33%", multi: "80.00%", group: "70.00%" },
      v0: { name: "Phiên bản v0 (Baseline)", desc: "Mã nguồn gốc ban đầu chưa chỉnh sửa. Còn lỗi thiếu thông tin và tự tạo ticket.", acc: "70.00% (21/30)", routing: "76.67%", multi: "80.00%", group: "60.00%" }
    };

    function onVersionChange() {
      const ver = document.getElementById('agentVersion').value;
      const data = versionData[ver] || versionData.v3;
      document.getElementById('cardVerName').innerText = data.name;
      document.getElementById('cardVerDesc').innerText = data.desc;
      document.getElementById('statCaseAcc').innerText = data.acc;
      document.getElementById('statRouting').innerText = data.routing;
      document.getElementById('statMulti').innerText = data.multi;
      document.getElementById('statGroup').innerText = data.group;
      resetSession();
    }
    ...
    function closeBenchmarkModal(e) {
    ...
    async function resetSession() {
      ...
      headerSubtitle.innerText = `Artifact: ${version} • Provider: ${provider} • Port 8501`;
